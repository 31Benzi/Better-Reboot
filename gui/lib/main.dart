  ...
  try {
    ...
    await runCatching(
        callable: () => localNotifier.setup(
            appName: 'Better Reboot Launcher',
    ...
    await runCatching(
        callable: () async {
          ...
          await Get.find<DiscordRpcService>().init();
        },
        errorFormatter: (error) => "Cannot initialize Discord Rich Presence: $error"
    );
    await runCatching(
        callable: () async {
          try {
...
class _RebootApplicationState extends State<RebootApplication> {
  ...
  Widget build(BuildContext context) => Obx(() => FluentApp(
        ...
        color: Colors.blue,
        theme: _createTheme(Brightness.dark),
