...
class _RebootPagerState extends State<RebootPager> with WindowListener, AutomaticKeepAliveClientMixin {
      ...
      onUpdate: (latestVersion) {
        ...
        infoBar = showRebootInfoBar(
            ...
            action: Button(
              ...
              onPressed: () {
                infoBar.close();
                launchUrl(Uri.parse("https://github.com/31Benzi/Better-Reboot/releases"));
              },
            )
        );
      }
  ...
  void onWindowClose() async {
    ...
    try {
      await DiscordRpcService.optional?.shutdown();
    }catch(error) {
      log("[DISCORD] Cannot shut down Rich Presence on exit: $error");
    }
  ...
  void onWindowFocus() {
    _focused.value = true;
    unawaited(DiscordRpcService.optional?.syncPresence(force: true));
  }
  ...
  Widget _buildBodyHeader() {
    final inactiveColor = Colors.grey[60];
    return Align(
