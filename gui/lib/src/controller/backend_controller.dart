...
class BackendController extends GetxController {
  ...
  AuthBackendType _readType() {
    final index = _storage?.read("type") ?? 0;
    if (index is! int || index < 0 || index >= AuthBackendType.values.length) {
      return AuthBackendType.local;
    }

    final stored = AuthBackendType.values.elementAt(index);
    return stored == AuthBackendType.embedded ? AuthBackendType.local : stored;
  }

  void reset() async {
    type.value = AuthBackendType.local;
